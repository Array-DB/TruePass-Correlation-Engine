...
def build_parser()->argparse.ArgumentParser:
    ...
    host=cs.add_parser("host");host.add_argument("--once",action="store_true",default=True)
    ...
    spectrum=cs.add_parser("sdr-spectrum");spectrum.add_argument("--source",choices=("synthetic","hackrf"),default="synthetic");spectrum.add_argument("--samples",type=int,default=4096);spectrum.add_argument("--serial",default=None)
    sub.add_parser("status"); verify=sub.add_parser("verify");vs=verify.add_subparsers(dest="verify_command"); ledger=vs.add_parser("ledger");ledger.add_argument("--path",default=None); dod=vs.add_parser("dod");dod.add_argument("--no-external",action="store_true",help="run source/runtime checks only")
...
def main(argv:list[str]|None=None)->int:
    ...
    if a.command=="verify":
        ...
        if a.verify_command=="dod":
            from truepass.verification import run_overall_dod_audit
            audit=run_overall_dod_audit(include_external=not a.no_external)
            print(json.dumps(audit.as_dict(),indent=2,sort_keys=True))
            return 0 if audit.overall_pass else 1
        p.error("choose: truepass verify ledger | truepass verify dod")
    if a.command=="collect":
        if a.collector=="processes":return _collect_processes(settings,a.once,a.interval)
        if a.collector=="host": print(json.dumps(HostTelemetryCollector().snapshot().as_dict(),indent=2,sort_keys=True));return 0
        ...
        if a.collector=="sdr-spectrum":
            c=settings.sdr; cfg=SDRConfig(c.sample_rate,c.center_frequency,c.gain,c.buffer_size,a.source)
            try:
                if a.source=="hackrf":
                    with HackRFOneSource(cfg,serial=a.serial) as src:
                        samples=src.read_samples(a.samples)
                else:
                    samples=SyntheticIQSource(cfg).read_samples(a.samples)
                if len(samples)<8: raise RuntimeError("not enough I/Q samples received")
                frame=SpectrumDSPPipeline(sample_rate_hz=cfg.sample_rate,center_frequency_hz=cfg.center_frequency).process(samples)
            except (RuntimeError,ValueError) as exc:
                print(f"SDR spectrum error: {exc}",file=sys.stderr);return 1
            print(json.dumps(frame.as_dict(),separators=(",",":")));return 0
