...
class SDRSettings(BaseModel):
    model_config=ConfigDict(extra="forbid"); sample_rate:float=2_000_000.0; center_frequency:float=100_000_000.0; gain:float=0.0; buffer_size:int=Field(default=4096,ge=1,le=10_000_000); device:str="synthetic"
...
def _apply_environment(data:dict[str,Any])->None:
    for key,raw in os.environ.items():
        if not key.startswith("TRUEPASS_") or key=="TRUEPASS_CONFIG":continue
